...
参考来源：CSDN 博客《C++实现轻量级RPC分布式网络通信框架》（作者 Yugang_Yang / T_Solotov），原文链接与参考源码（<https://github.com/yyg192/yyg_rpc_server>）标注在 `docs/01` 顶部。笔记中由代码截图整理的代码块均带有 `// 依据原文整理` 注释。
...
以下设计来自 `docs/01`，是未来实现的理论参考，**尚不存在对应代码**。命名（如 `Mprpc*`）沿用参考项目，正式实现时可能变化。
...
参考项目未实现、KopiRPC 计划迭代的方向（见 `docs/01` 知识点14）：客户端异步调用、负载均衡、异常重试、健康检测、熔断机制、长连接/连接池。README 中的 Roadmap 一节目前仅写"尽情期待"。

## 仓库约定（重要）

1. **README.md 严格面向读者**：只写什么是 RPC、为什么需要、应用场景、Roadmap、笔记链接。实现细节（调用流程图、组件内部、技术表格、代码）**不得写入 README**，它们属于学习笔记。这是用户的明确规则。
2. **docs/ 目录**存放编号学习笔记（`01-`、`02-`……），通过学习材料（文章、截图、视频）生成，格式约定：
   - 顶部标注来源（链接 + 作者）
